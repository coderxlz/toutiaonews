<template>
  <div>
    <van-nav-bar title="登录" left-arrow validate-trigger="onSubmit" />
    <!-- 提交表单部分 -->
    <van-form
      @submit="onSubmit"
      @failed="subFail"
      ...
      >
        <template #button>
          ...
          <van-button
            ...
            type="primary"
            native-type="button"
            @click="sendCode(user.mobile)"
            :loading="ifSendCodeLoading"
            >发送验证码</van-button
          >
  ...
  methods: {
    ...
    async onSubmit() {
      ...
      try {
        ...
        // 从url中得到之前页面的url，跳转会之前浏览的页面，如果没有之前的页面路径，则默认跳转到主页
